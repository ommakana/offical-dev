...
export function QuoteBar() {
  return (
    <div
      className="border-b border-surface-border bg-indigo-50/60 dark:bg-indigo-950/20"
      role="complementary"
      aria-label="Quote of the day"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2.5">
        <p className="flex items-start gap-2 text-xs text-slate-500 dark:text-slate-400 italic leading-relaxed">
          <QuoteIcon />
          <span>
            {text}
            {author && (
              <span className="not-italic font-medium text-slate-400 dark:text-slate-500 ml-1.5">
                — {author}
              </span>
            )}
          </span>
